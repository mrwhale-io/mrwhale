...
export default class extends GameJoltCommand {
  constructor() {
    super({
      name: "mystatus",
      description: "Check your subscription status, usage limits, and premium features.",
      type: "subscription",
  ...
  async action(message: Message): Promise<Message> {
    ...
    try {
      const subscription = await this.botClient.subscriptionManager.getUserSubscription(message.user.id);
      const limits = await this.botClient.subscriptionManager.getUserPremiumLimits(message.user.id);
      ...
      if (!subscription) {
        ...
        statusText += `• Advanced Effects: ${limits.hasAdvancedEffects ? '✅' : '❌'}\n`;
        statusText += `• AI Effects: ${limits.hasAIEffects ? '✅' : '❌'}\n`;
        statusText += `• Animated Effects: ${limits.hasAnimatedEffects ? '✅' : '❌'}\n\n`;
        ...
        statusText += `• Expired: ${new Date(subscription.startDate).toLocaleDateString()}\n\n`;
        ...
        statusText += `• Status: ${this.getStatusIcon(subscription.status)} ${subscription.status.toUpperCase()}\n`;
        statusText += `• Started: ${new Date(subscription.startDate).toLocaleDateString()}\n`;
        
        if (subscription.nextBillingDate) {
          statusText += `• Next Billing: ${new Date(subscription.nextBillingDate).toLocaleDateString()}\n`;
        }
        
        if (subscription.lastPaymentDate) {
          statusText += `• Last Payment: ${new Date(subscription.lastPaymentDate).toLocaleDateString()}\n`;
        }
        
        statusText += "\n**Your Premium Features:**\n";
        statusText += `• Custom Commands: ${limits.maxCustomCommands === -1 ? 'Unlimited' : limits.maxCustomCommands}\n`;
        statusText += `• Daily Image Effects: ${limits.maxImageEffectsPerDay === -1 ? 'Unlimited' : limits.maxImageEffectsPerDay}\n`;
        statusText += `• Advanced Effects: ${limits.hasAdvancedEffects ? '✅' : '❌'}\n`;
        statusText += `• AI Effects: ${limits.hasAIEffects ? '✅' : '❌'}\n`;
        statusText += `• Animated Effects: ${limits.hasAnimatedEffects ? '✅' : '❌'}\n\n`;
        ...
        statusText += "• Custom Commands: Check `!listcommands` for your commands\n\n";
      ...
      this.botClient.logger.error(`Failed to get status for user ${message.user.id}:`, error);
      return message.reply(
